...
for epsilon in [0.05]:
    ...
    for lc in range(dotimes):
        ...
        class PhysicsInformedNN:
            def __init__(self, x, layers, allfcn, input_weight = np.array([0]), learning_rate_cus=0.001, weight_loss=[1,1,1], method="no", mode="vw"):
                ...
                config = tf.compat.v1.ConfigProto(
                    intra_op_parallelism_threads = 20,  # 設定單個操作的並行線程數
                    inter_op_parallelism_threads = 20,  # 設定多個操作之間的並行線程數
                    device_count={"CPU":10},
                ...
                if mode == "vw":
                    ...
                    self.y_loss = self.net_NS_u(self.x_tf, self.eps, self.afcn, self.bfcn, weight_loss)
                    self.loss_r = self.y_loss[2]
            ...
            def neural_net_relative(self, x, weights, biases):
                ...
                H = tf.convert_to_tensor(x, tf.float32)
                U = tf.convert_to_tensor(x, tf.float32)
                V = tf.convert_to_tensor(x, tf.float32)
                W1 = weights[0]
                b1 = biases[0]
                W2 = weights[0]
                b2 = biases[0]
                U = tf.math.tanh(tf.add(tf.matmul(U, W1), b1))
                V = tf.math.tanh(tf.add(tf.matmul(V, W2), b2))
            ...
            def net_NS_u(self, x, eps, afcn, bfcn, weights):
                ...
                g0u = tf.map_fn(self.g0fcn, x[int(self.x.shape[0] * self.l0):])
                g0u = tf.reshape(g0u, shape=[-1,1])
                g1u = tf.map_fn(self.g0fcn, x[:int(self.x.shape[0] * self.l1)])
                g1u = tf.reshape(g1u, shape=[-1,1])

                y_bddl = self.mu0 + self.simp(g0u*y[int(self.x.shape[0] * self.l0):], self.x[int(self.x.shape[0] * self.l0):]) / eps
                y_bddr = self.mu1 + self.simp(g1u*y[:int(self.x.shape[0] * self.l1)], self.x[:int(self.x.shape[0] * self.l1)]) / eps
            ...
            def train(self, nIter):
                ...
                if self.method == "yes":
                    xf_dict = {self.x_tf: self.x, self.adaptive_constant_tf: self.adaptive_constant_val}
                else:
                    xf_dict = {self.x_tf: self.x}
...
#=================================================================MAIN=================================================================#
        if __name__ == "__main__": 
            ...
            train_times = 20000
            if lc == 0:
                # weight_loss_v = [100, 1, 1]
                weight_loss_v = [1,1,1,1,1]  # u
                model = PhysicsInformedNN(x_train, layers, allfcn, learning_rate_cus = learning_rate_cus, weight_loss=weight_loss_v, method=method, mode=mode)
            else:
                weight_loss_w = [1, 1, 1]
                model = PhysicsInformedNN(x_train, layers, allfcn, learning_rate_cus = learning_rate_cus, weight_loss=weight_loss_w, method=method, mode=mode)
    ...
    with open(folder_path + '/%s/record_%s.txt'%(date_string,date_string), 'w') as f:
        ...
        if mode == "vw":
            ...
            f.write('\nweight of left boundary point_aux_v : %s\n'%weight_loss_v[4])
...
for item in epsilon_save.items():
    ...
    plt.plot(test_points, ub_exact[0]*PCal.V(test_points,Cv,eps) + ub_exact[1]*PCal.W(test_points,Cw,eps))
...
for item in epsilon_save.items():
    ...
    v_exact = PCal.V(test_points, item[1]["Cvw"][0], eps)
    w_exact = PCal.W(test_points, item[1]["Cvw"][1], eps)
